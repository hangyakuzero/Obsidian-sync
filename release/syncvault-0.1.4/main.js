...
// src/ui/RebuildModal.ts
var import_obsidian3 = require("obsidian");
var RebuildModal = class extends import_obsidian3.Modal {
  constructor(app, state, client, mode, onDone, beforeReset) {
    super(app);
    this.state = state;
    this.client = client;
    this.mode = mode;
  ...
  onOpen() {
    ...
    contentEl.createEl("p", {
      text: rebuild ? "This device's files become the new vault baseline. The server's sync history for this vault is wiped (nothing on any device is deleted), then every local file is uploaded with its real content." : "This device downloads the rebuilt vault contents from the server. Local files on this device are not deleted, but files present in the rebuilt vault will overwrite local copies."
    });
    contentEl.createEl("p", {
      text: `Vault: ${(_b = (_a = this.state.vaultName) != null ? _a : this.state.vaultId) != null ? _b : "?"}`
    });
    if (rebuild) {
    ...
    new import_obsidian3.Setting(contentEl).addButton(
      (b) => b.setButtonText(rebuild ? "Rebuild (wipes server history)" : "Join rebuilt vault").setWarning().onClick(() => void this.submit())
    ).addButton(
  ...
  async submit() {
    ...
    try {
      ...
      if (rebuild) {
        ...
        new import_obsidian3.Notice("SyncVault: server history wiped \u2014 uploading files...", 6e3);
      } else {
        await this.state.save({
          lastRevision: 0,
          seeded: true,
        ...
        new import_obsidian3.Notice("SyncVault: joining rebuilt vault \u2014 downloading...", 6e3);
      }
      this.close();
      this.onDone();
    } catch (e) {
...
var SyncVaultSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Server URL").addText((t) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Account").setDesc((_a = this.state.accountId) != null ? _a : "");
    new import_obsidian4.Setting(containerEl).setName("Vault").setDesc((_c = (_b = this.state.vaultName) != null ? _b : this.state.vaultId) != null ? _c : "");
    new import_obsidian4.Setting(containerEl).setName("Device").setDesc((_e = (_d = this.state.deviceName) != null ? _d : this.state.deviceId) != null ? _e : "");
    new import_obsidian4.Setting(containerEl).setName("Status").setDesc(
    ...
    if (this.engine.isPaused) {
      new import_obsidian4.Setting(containerEl).setName("Sync paused").setDesc("A remote change could not be applied, so polling stopped.").addButton(
        (b) => b.setButtonText("Resume sync").setCta().onClick(() => {
          void this.engine.resume().then(() => this.display());
        })
      );
    }
    new import_obsidian4.Setting(containerEl).setName("Rescue").setDesc("Recover a vault whose server history is corrupted (e.g. old builds uploaded files without content).").addButton(
      (b) => b.setButtonText("Rebuild vault from this device").setWarning().onClick(() => {
      ...
      (b) => b.setButtonText("Join rebuilt vault").setWarning().onClick(() => {
    ...
    new import_obsidian4.Setting(containerEl).addButton(
      (b) => b.setButtonText("Disconnect vault").onClick(async () => {
        ...
        new import_obsidian4.Notice("SyncVault: vault disconnected");
        this.display();
      })
    );
  }
  renderWelcome(parent) {
    ...
    new import_obsidian4.Setting(parent).addButton(
...
var SyncState = class {
  ...
  validQueuedChange(change) {
    ...
    if ((item.operation === "create" || item.operation === "update") && (typeof item.payload !== "string" || !isValidBase64(item.payload))) {
      return false;
    }
  ...
  async enqueue(change) {
    ...
    for (const key of touched) {
      for (let i = pending.length - 1; i >= 0; i--) {
        const c = pending[i];
        if ((c.path === key || c.oldPath === key) && !(preserveRename && c.operation === "rename" && c.path === change.path)) {
          pending.splice(i, 1);
        }
      }
    }
...
var SyncConnection = class {
  ...
  handle(msg) {
    switch (msg.type) {
      case "welcome":
        this.setStatus("open");
        this.remoteChain = this.remoteChain.then(() => this.callbacks.onWelcome(msg.serverRevision, msg.resyncRequired)).catch(() => void 0);
        break;
      case "change":
        this.remoteChain = this.remoteChain.then(() => this.callbacks.onRemoteChange(msg.change)).catch(() => void 0);
        break;
      case "accepted":
...
var HttpConnection = class {
  ...
  async pull(since) {
    ...
    try {
      ...
      if (isApiError(e) && e.status === 401) {
        (_f = (_e = this.callbacks).onAuthFailure) == null ? void 0 : _f.call(_e, "authentication expired; reconnect the vault");
      }
      throw e;
    }
  }
  ...
  async push(change, p) {
    ...
    try {
      ...
      if (isApiError(e) && e.status === 401) {
        (_b = (_a = this.callbacks).onRetry) == null ? void 0 : _b.call(_a, change.operationId, e.message);
        (_d = (_c = this.callbacks).onAuthFailure) == null ? void 0 : _d.call(_c, "authentication expired; reconnect the vault");
        return;
      }
      if (isApiError(e) && e.status >= 400 && e.status < 500) {
        (_f = (_e = this.callbacks).onRejected) == null ? void 0 : _f.call(_e, change.operationId, e.code, e.message);
        return;
      }
      (_h = (_g = this.callbacks).onRetry) == null ? void 0 : _h.call(_g, change.operationId, e.message);
...
var SyncEngine = class {
  constructor(state, queue, watcher, vault, onStatus, onNotice, options = {}) {
    ...
    const handlers = {
      ...
      onConflict: (c) => this.settleAck(c.operationId, { status: "conflict", conflictPath: c.conflictPath }),
  ...
  handleAuthFailure(message) {
    ...
    this.onNotice(`SyncVault: sync paused \u2014 ${message}. Reconnect the vault to continue.`, 1e4);
  }
  ...
  async applyRemoteChange(change) {
    ...
    try {
      ...
      this.setStatus("paused");
      this.onNotice(
        `SyncVault: sync paused \u2014 could not apply "${change.path}": ${e.message}. Fix the issue, then resume sync (Settings \u2192 Resume).`,
        1e4
      );
      return;
    } finally {
...
var SyncVaultPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.engine = new SyncEngine(
      ...
      (message, timeout) => this.notify(message, timeout),
      {
        ...
        scanner: {
          listFiles: async () => {
            const files = this.app.vault.getAllLoadedFiles().filter((f) => f instanceof import_obsidian5.TFile);
  ...
  notify(message, timeout) {
    ...
    new import_obsidian5.Notice(message, timeout != null ? timeout : 5e3);
  }
  async onload() {

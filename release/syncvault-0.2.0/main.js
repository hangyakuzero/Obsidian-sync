...
var RecoverModal = class extends import_obsidian3.Modal {
  constructor(app, state, client, mode, onDone, beforeReset, enterJoin, countSyncable) {
    super(app);
    this.state = state;
    this.client = client;
    this.mode = mode;
  ...
  async submit() {
    ...
    try {
      ...
      if (reset) {
        ...
        new import_obsidian3.Notice("SyncVault: pulling rebuilt baseline \u2014 local files are preserved...", 6e3);
      }
      this.close();
      this.onDone();
    } catch (e) {
...
var SyncVaultSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Recover sync").setDesc("Last resort when the server history is unusable (e.g. old builds uploaded files without content).").addButton(
...
var HttpConnection = class {
  ...
  async pull(since) {
    ...
    try {
      ...
      if (isApiError(e) && e.status === 401) {
        ...
        (_j = (_i = this.callbacks).onError) == null ? void 0 : _j.call(
          _i,
  ...
  async fetchContent(change) {
    ...
    try {
      ...
      (_b = (_a = this.callbacks).onError) == null ? void 0 : _b.call(_a, `content download failed: ${e.message}`);
      return null;
    }
  }
  async push(change, bytes, p) {
    ...
    try {
      ...
      if (isApiError(e) && e.code === "RESYNC_REQUIRED") {
        (_h = (_g = this.callbacks).onResyncRequired) == null ? void 0 : _h.call(_g);
        (_j = (_i = this.callbacks).onError) == null ? void 0 : _j.call(_i, e.message);
        return;
      }
      if (isApiError(e) && e.status >= 400 && e.status < 500) {
        (_l = (_k = this.callbacks).onRejected) == null ? void 0 : _l.call(_k, change.operationId, e.code, e.message);
        return;
      }
      (_n = (_m = this.callbacks).onRetry) == null ? void 0 : _n.call(_m, change.operationId, e.message);
      (_p = (_o = this.callbacks).onError) == null ? void 0 : _p.call(_o, `push failed: ${e.message}`);
    }
  }
  params() {
    var _a, _b, _c, _d;
...
var SyncEngine = class {
   ...
   */
  async ensureNoBlockingFile(path, change) {
  ...
  async enqueueConflictCopy(path, bytes) {
    const operationId = this.newOperationId();
  ...
  async backupIfDivergent(path, change, incoming) {
    const kind = await this.vault.stat(path);
    if (kind !== "file") return;
  ...
  async freshLocalConflictPath(path, change) {
    ...
    const stamp = new Date(change.timestamp || Date.now()).toISOString().replace(/[-:T]/g, "").slice(0, 12);
    for (let i = 0; i < 100; i++) {
      const suffix = i === 0 ? "" : `-${i}`;
    ...
    try {
      for (const item of [...this.queue.items]) {
        ...
        if (result.status === "accepted") {
          ...
          if (item.stagedFile) await ((_d = this.staging) == null ? void 0 : _d.remove(item.stagedFile));
          this.setStatus("conflict");
        }
      }
    } finally {
